...
from .simple_auth import simple_client
from .config import settings


def setup_todo_tools(server: Server):
    """Setup todo-related tools using personal token authentication"""
    ...
    async def handle_call_tool(name: str, arguments: Dict[str, Any]) -> List[TextContent]:
        ...
        try:
            if name == "create_todo":
                ...
                result = await simple_client.create_todo(todo_data)
                return [TextContent(
                    type="text",
                ...
                todos = await simple_client.list_todos()
                if not todos:
                ...
                todo = await simple_client.get_todo(arguments["todo_id"])
                return [TextContent(
                    type="text",
                ...
                result = await simple_client.update_todo(todo_id, update_data)
                return [TextContent(
                    type="text",
                ...
                result = await simple_client.delete_todo(arguments["todo_id"])
